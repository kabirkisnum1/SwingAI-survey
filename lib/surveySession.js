...
function createSession(deviceId) {
  ...
  const picks = catalog.picksPerSession || { reddit: 4, og: 4, golfdb: 2 };
  const minGap = catalog.golfdbMinIdGap || 8;
  const usage = loadUsage();
  const usedKeys = new Set();

  const reddit = pickFromPool(catalog.pools.reddit || [], picks.reddit, usage, deviceId, usedKeys);
  const og = pickFromPool(catalog.pools.og || [], picks.og, usage, deviceId, usedKeys);
  const golfdb = pickGolfdbPair(catalog.pools.golfdb || [], usage, deviceId, usedKeys, minGap);

  const all = [...reddit, ...og, ...golfdb];
  const expected = (picks.reddit || 0) + (picks.og || 0) + (picks.golfdb || 0);
